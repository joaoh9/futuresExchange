...
exports.getTicker = async (conn, instumentName = null) => {
  ...
  if (instumentName) {
    ...
    ticker = await conn.fetchTickers();
  }
...
exports.fetchBidsAsks = async (conn, instumentName = null) => {
  ...
  if (instumentName) {
    ...
    bidsAsks = await conn.fetchBidsAsks();
  }
...
exports.filterPrice = (price, precision) => {
  return parseFloat(
    decimalToPrecision(numberToString(price), TRUNCATE, precision, DECIMAL_PLACES, NO_PADDING)
  );
};

exports.filterQuantity = (quantity, precision) => {
  return parseFloat(
    decimalToPrecision(numberToString(quantity), TRUNCATE, precision, DECIMAL_PLACES, NO_PADDING)
  );
};
...
exports.getAbsoluteValueByExchange = (value, type, instrumentName, exchangeMarkets) => {
...
exports.getRelativeValuesAndPrecisionByExchange = (
  ...
  const exchangePrecision = exchangeMarkets ? exchangeMarkets[instrumentName].precision : null;
  let relativeValue = 0;
  ...
  relativeValue = Math.round(value * relativePrecision);

  return { relativeValue, relativePrecision };
};

exports.getInstrumentId = (instrumentName, exchangeMarkets) => exchangeMarkets[instrumentName].id;
